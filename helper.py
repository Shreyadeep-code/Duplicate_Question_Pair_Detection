...
def test_common_words(q1,q2):
    w1=set(map(lambda x : x.strip().lower(),q1.split(' ')))
    w2=set(map(lambda x : x.strip().lower(),q2.split(' ')))
    return len(w1&w2)
...
def test_fetch_token_features(q1,q2):
    ...
    if len(q1_tokens)==0 or len(q2_tokens)==0: return token_features
...
def create_query_point(q1,q2):
    ...
    q1_bow=vec.transform([q1]).toarray()
    q2_bow=vec.transform([q2]).toarray()
